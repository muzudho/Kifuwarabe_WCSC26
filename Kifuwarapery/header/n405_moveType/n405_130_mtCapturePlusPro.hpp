...
public:
	...
	MoveStack* GenerateMove(MoveStack* moveStackList, const Position& pos, bool all = false
		...
		{
			moveStackList = MoveTypeCapturePlusPro::GENERATE_MOVE_<Color::Black, Color::White>(moveStackList, pos, all);
		}
		else
		{
			moveStackList = MoveTypeCapturePlusPro::GENERATE_MOVE_<Color::White, Color::Black>(moveStackList, pos, all);
		}
...
private:
	...
	static inline MoveStack* GENERATE_MOVE_(MoveStack* moveStackList, const Position& pos, bool all = false
		...
		const Bitboard targetPawn = pos.GetBbOf10(THEM) | (pos.GetOccupiedBB().NotThisAnd(TRank789BB));
