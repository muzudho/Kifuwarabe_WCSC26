...
class MoveTypeNonCapture : public MoveTypeAbstract {
public:
	// 指し手生成 functor
	...
	MoveStack* GenerateMove(MoveStack* moveStackList, const Position& pos, bool all = false
		...
		{
			moveStackList = MoveTypeNonCapture::GENERATE_MOVE_<Color::Black, Color::White>(moveStackList, pos, all);
		}
		else
		{
			moveStackList = MoveTypeNonCapture::GENERATE_MOVE_<Color::White, Color::Black>(moveStackList, pos, all);
		}
...
private:
	...
	static inline MoveStack* GENERATE_MOVE_(MoveStack* moveStackList, const Position& pos, bool all = false
		...
		const PieceMoveEvent pmEvent(MoveType::N01_NonCapture, all, pos, ksq);
