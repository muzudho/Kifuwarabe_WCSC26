...
public:
	...
	MoveStack* MakeDropMovesToRank8ExceptN(
		...
		while (toBB.GetP(0)) {
			...
			pMovestack->m_move = ConvMove::Convert30_MakeDropMove_da(haveHandArr[dmEvent.m_noKnightIdx + 4], iTo);
			pMovestack++;
			pMovestack->m_move = ConvMove::Convert30_MakeDropMove_da(haveHandArr[dmEvent.m_noKnightIdx + 3], iTo);
			pMovestack++;
	...
	MoveStack* MakeDropMovesToRank1234567(
		MoveStack* pMovestack,
		const DropMakerEvent& dmEvent,
		Move haveHandArr[6]// TODO: これ、打ムーブにできないかだぜ☆？（＾ｑ＾）
	) const override {
